...
/** Reference-based cargo truck, in metres with +Z forward. */
export function createIndianTruck(): T.Group {
  const truck = new T.Group();
  truck.name = 'Rajasthan Express';
  ...
  function mesh(name: string, geometry: T.BufferGeometry, material: string, x: number, y: number, z: number, parent = truck) {
  ...
  function box(name: string, w: number, h: number, d: number, mat: string, x: number, y: number, z: number) { return mesh(name, new T.BoxGeometry(w,h,d), mat,x,y,z); }
  function rounded(name: string,w: number,h: number,d: number,mat: string,x: number,y: number,z: number,r=.08) { return mesh(name,new T.BufferGeometry().copy(new RoundedBoxGeometry(w,h,d,3,r)),mat,x,y,z); }
  function cyl(name: string, r: number, h: number, mat: string, x: number,y: number,z: number) { return mesh(name, new T.CylinderGeometry(r,r,h,24),mat,x,y,z); }
  function label(text: string, size: number, mat: string, x: number,y: number,z: number, rotation = 0): T.Mesh {
    const artwork = TextureGenerator.createTruckLetteringTexture(text, materials[mat].color.getStyle());
    const height = size * 1.45;
    const geometry = new T.PlaneGeometry(height * artwork.aspect, height);
    const material = new T.MeshStandardMaterial({ map: artwork.texture, transparent: true, depthWrite: false, roughness: .6 });
    const sign = new T.Mesh(geometry, material);
    sign.name = text;
    sign.position.set(x, y + size * .48, z);
    sign.rotation.y = rotation;
    truck.add(sign);
    return sign;
  }
  function rod(name: string,a: number[],b: number[],r: number,mat: string) {
    const av = new T.Vector3(...a), bv = new T.Vector3(...b), delta = bv.clone().sub(av);
    const m = cyl(name,r,delta.length(),mat,av.clone().add(bv).multiplyScalar(.5).x, (av.y+bv.y)/2, (av.z+bv.z)/2); m.quaternion.setFromUnitVectors(new T.Vector3(0,1,0),delta.normalize()); return m;
  }
